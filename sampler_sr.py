...
DIFFUSERS_SRC = PROJECT_ROOT / "src"
if DIFFUSERS_SRC.is_dir():
    diffusers_src = str(DIFFUSERS_SRC)
...
_positive= 'High-resolution satellite image of the same scene, preserving all cloud structures and patterns, sharper and more detailed clouds, clean edges, no added objects, no distortions, minimal noise, physically realistic appearance.'
_negative= 'cartoon, painting, unrealistic colors, 3D render, text, logos, checkerboard artifacts, ringing, blocky compression, over-smoothed or deformed clouds, hallucinated land/sea, duplicated patterns, noise, speckles, sensor glitches'
...
class SuperResolutionSampler(BaseSampler):
    @torch.no_grad()
    def sample_func(self, im_cond, return_tensor=False):
        '''
        Input:
            im_cond: b x c x h x w, torch tensor, [0,1], RGB
        Output:
            xt: h x w x c, numpy array, [0,1], RGB, or a GPU NCHW tensor
                when return_tensor is True
        '''
        im_cond_color_ref = im_cond
        ...
        if self.configs.color_fix:
            im_cond_up = F.interpolate(
                im_cond_color_ref, size=res_sr.shape[-2:], mode='bicubic', align_corners=False, antialias=True
            )
            ...
            elif self.configs.color_fix == 'rgb':
                res_sr = util_color_fix.rgb_luminance_preserve(res_sr, im_cond_up)
            else:
                raise ValueError(f"Unsupported color fixing type: {self.configs.color_fix}")

        res_sr = res_sr.clamp(0.0, 1.0)
        if return_tensor:
            return res_sr

        res_sr = res_sr.cpu().permute(0,2,3,1).float().numpy()
