...
def create_dataset(dataset_config):
    ...
    elif  dataset_config['type'] == 'realesrgan_paired':
        dataset = RealESRGANPairedDataset(dataset_config['params'])
...
class BaseData(Dataset):
    ...
    def _resolve_extra_image_path(self, im_path_base):
        base_path = Path(im_path_base)
        candidates = [Path(self.extra_dir_path) / base_path.name]

        stem_parts = base_path.stem.split("_")
        if len(stem_parts) > 1:
            sample_idx = stem_parts[-1]
            candidates.extend([
                Path(self.extra_dir_path) / f"viirs_{sample_idx}.png",
                Path(self.extra_dir_path) / f"mtg_{sample_idx}.png",
            ])
        ...
        raise FileNotFoundError(f"Could not resolve extra image for {im_path_base}. Checked: {candidate_str}")

    def __getitem__(self, index):
        im_path_base = self.file_paths[index]
        #im_base = util_image.imread(im_path_base, chn='rgb', dtype='float32')
        #im_base = Image.open(im_path_base).convert('RGB')    #util_image.imread(im_path_base, chn='rgb', dtype='float32')
        #im_base = util_image.imread(im_path_base, chn='rgb', dtype='float32', size=(128,128))
        #im_target = self.transform(im_base)
        im_base = util_image.imread(im_path_base, chn='bgr', dtype='float32')
        im_target = img2tensor([im_base], bgr2rgb=True, float32=True)[0]
        out = {'image':im_target, 'lq':im_target}

        if self.extra_dir_path is not None:
            im_path_extra = self._resolve_extra_image_path(im_path_base)
