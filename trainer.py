...
# _positive = 'High-resolution satellite image of the same scene, preserving all cloud structures and patterns, sharper and more detailed clouds, ' +\
#                 'clean edges, no added objects, no distortions, minimal noise, physically realistic appearance.'
_positive = (
    'High-fidelity meteorological satellite imagery, VIIRS sensor quality, '
    'sharp convective cloud tops, physically accurate radiance, '
    'distinct microphysical textures, highly detailed storm structure, '
    'scientific visualization, lossless, crisp granular details.'
)

_negative = (
    'Blurry, low resolution, interpolation artifacts, over-smoothed, '
    'cartoonish, oil painting style, deformed cloud shapes, unnatural gradients, '
    'checkerboard pattern, chromatic aberration, noisy, jpeg artifacts, '
    'non-physical features, hallucinated landmasses.'
)
...
class TrainerBaseSR(TrainerBase):
    ...
    def prepare_data(self, data, phase='train'):
        if phase == 'train' and self.configs.data.get(phase).get('type') == 'realesrgan_paired':
